# Feature Specification: Global Fixed Chat Widget (Corner Bubble)

**Feature Branch**: `004-global-chat-widget`  
**Created**: 2025-12-06
**Status**: Draft  
**Input**: User description: "Feature: Global Fixed Chat Widget (Corner Bubble) Goal: Implement a minimal, professional, togglable chat interface that appears on ALL pages of the Docusaurus site. Requirements: Component Creation: Create a new React component (GlobalChatWidget.tsx) in web/src/components/. Global Integration (Swizzle): The component MUST be imported and rendered in the Docusaurus global layout file (e.g., src/theme/Layout/index.tsx) to ensure it appears on the homepage and all book pages. Styling & Positioning: The component must have fixed positioning (position: fixed; bottom: 20px; right: 20px). It should initially display as a small, professional bubble/icon and toggle open to reveal the chat box. API Connection: The chat input must send a POST request to the local API at http://127.0.0.1:8000/ask. Data Sent: The request payload must include: question, chapter_id (hardcoded for the demo), and selected_text (optional, for future proofing). Display: Display the raw JSON response from the API (retrieved chunks) to the user. Cleanup: Remove any leftover placeholder chat components from web/src/pages/index.tsx."
...
- **FR-007**: Any leftover placeholder chat components from `web/src/pages/index.tsx` MUST be removed.
